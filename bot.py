...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cat:
        ...
        # REJA TUZISH BOSQICHI
        await update.message.reply_text("⏳ Sun'iy intellekt mavzu bo'yicha 4 ta reja tayyorlamoqda, kuting...")
        try:
            plans = await asyncio.to_thread(generate_plans, topic, user_lang)
            context.user_data['plans'] = plans
            
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Reja maqul, davom etish", callback_data="approve_plan")],
                [InlineKeyboardButton("🔄 Boshqa reja yaratish", callback_data="regenerate_plan")]
            ])
            
            await update.message.reply_text(
                f"📋 **Taklif etilgan rejalar:**\n\n{plans}\n\nUshbu reja sizga maqulmi?",
                reply_markup=keyboard, parse_mode="Markdown"
            )
        except Exception as e:
            await update.message.reply_text(f"Xatolik yuz berdi: {e}")
    else:
        await update.message.reply_text("⚠️ Avval menyudan bo'limni tanlang.", reply_markup=build_categories_keyboard())
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "approve_plan":
        await query.edit_message_text("✅ Reja tasdiqlandi! Fayl tayyorlanmoqda, iltimos kuting...")
        ...
        try:
            ...
            with open(p, 'rb') as f:
                await context.bot.send_document(chat_id=uid, document=f, reply_markup=build_categories_keyboard())
            os.remove(p)
            context.user_data.clear()
        except Exception as e:
            update_balance(uid, price)
            await context.bot.send_message(chat_id=uid, text=f"Fayl yaratishda xato: {e}", reply_markup=build_categories_keyboard())
...
if __name__ == '__main__':
    ...
    print("Bot reja tuzish va zamonaviy dizayn funksiyalari bilan ishga tushdi...")
    app.run_polling()
